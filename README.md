...
Run `python3 audit_questions.py` to get a breakdown of potentially problematic STACK questions and images.
In particular:
...
- Images referenced in the textbook source that that don't exist (causing a broken image)

### Permalinks

Run `python3 add_labels.py` to add xml IDs to all elements that don't have them. These labels will be used
for the generation of permalinks (and review links) so that these don't change across compilations of the textbook.
